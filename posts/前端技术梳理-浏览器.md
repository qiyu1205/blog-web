...
> 1. 什么是事件委托？
>    
>    在事件发生DOM元素的父元素上设置监听函数，通过事件冒泡，父元素可以监听到自元素上事件的触发，通过判断发生DOM元素的类型来做不同的处理。
> 
> 2. 事件委托的好处及应用场景
>    
>    好处：节省内存，不需要给子节点注销事件
>    
>    适合场景：比较适合动态元素的绑定，新增的子元素也会有监听函数。
> 
> 3. 阻止事件冒泡的方法
>    
...
ajax 请求时，浏览器要求当前网页和 server 必须同源（安全），**协议、域名和端口**有一个不同就是跨域。
...
1. 兼容性好，但只限于 `get` 请求
   ...
   `JSONP` 利用的是 `<script>` 标签没有跨域限制的漏洞，通过 `<script>` 标签指向一个需要访问的地址并提供一个回调函数来接收数据当需要通讯时。
